...
​		MVCC多版本并发控制指的是维持一个数据的多个版本，使得读写操作没有冲突，快照读是MySQL为实现MVCC的一个非阻塞读功能。MVCC模块在MySQL中的具体实现是由`三个隐式字段`，`undo日志`、`read view`三个组件来实现的。
...
![数据案例](https://cdn.wuzx.cool/%E6%95%B0%E6%8D%AE%E6%A1%88%E4%BE%8B.png)
